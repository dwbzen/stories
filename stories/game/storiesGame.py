...
class StoriesGame(StoriesObject):
    ...
    @property
    def story_discard_deck(self)->CardDeck:
    ...
    def add_player(self, aplayer:Player):
        """Adds a new Player to the game.
            This also deals deal_size number of story cards to the player
        ...
        aplayer.story_card_hand.add_cards(cards)
        aplayer.game_id = self._game_id
    ...
    def draw_card(self, what:str, types_to_omit:List[CardType])->tuple() :
        """Draw a card from the deck OR from the top of the global discard deck
            Arguments:
                what - 'new' : draw a card from the main deck. 'discard' : draw the top card from the global discard deck
        ...
        elif self._story_discard_deck.size() > 0:
            #
